...
export LUMBERROOM_BUILD_TAG="${LUMBERROOM_BUILD_TAG:-lumberroom-server:0.2.0}"
...
if [ "$BUILD_LOCAL" = 1 ]; then
  ...
  info "pulling ${LUMBERROOM_SERVER_IMAGE:-ghcr.io/the-cybersapien/lumberroom-server:0.2.0} (a few seconds to a minute)"
  if [ "$DRY_RUN" = 1 ]; then
