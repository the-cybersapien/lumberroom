...
Both variables are read by `docker-compose.yml` and default to `0.4.0`. Neither tracks `latest`: a
...
Pin the new version in `.env` too, or the next `up -d` with no override falls back to `0.4.0`.
