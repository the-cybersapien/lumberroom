...
#
# Three failures on this box are invisible to `curl /healthz`, and each has cost real time:
#
...
#   with a 403 the client reports as a connection failure. Step 3 is the only one that catches it,
...
#   client's discovery fails. Step 5 compares the documents against the URL you asked about.

set -euo pipefail

URL="${1:-${LUMBERROOM_URL:-}}"
URL="${URL%/}"
TOKEN="${LUMBERROOM_TOKEN:-}"

[ -n "$URL" ] || { echo "usage: LUMBERROOM_TOKEN=... $0 https://memory.example.com" >&2; exit 1; }
...
say "2/6 it is ready, and its key is the one this store was sealed with"
if [ "$(get /readyz "$WORK/ready.json")" = 200 ]; then
  ...
  [ "$(field "$WORK/ready.json" ok)" = "true" ] \
    ...
    || fail "ok is not true"
  KEK="$(field "$WORK/ready.json" kek_provider)"
  VERIFIED="$(field "$WORK/ready.json" kek_verified)"
  if [ "$KEK" = "none" ]; then
...
say "3/6 a real MCP tool call, which is the check the others cannot make"
if [ -z "$TOKEN" ]; then
...
say "4/6 the credential resolves to the grant you meant"
if [ -z "$TOKEN" ]; then
...
say "5/6 the issuer agrees with the host clients reach"
...
say "6/6 the transport is one a browser surface will accept"
case "$URL" in
